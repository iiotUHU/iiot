...
const int N_MEDIDAS_CAL=10; //numero de medidas para la calibracion
const int N_MEDIDAS_USO=5; //numero de medidas para la calibracion
const int N_MODAS=400;//numero de medidas posibles en la calibracion
...
const char escenarios [ESCENARIOS_CALIBRAR][6]={"VACIO","ALTO","BAJO"}; //nombre de los escenarios a calibrar
...
int medidas [N_MEDIDAS_CAL];//arreglo para calcular media y mediana
int media; 
int valoresModa[N_MODAS]; //ya que los valores se encuentran entre 2 y 400, cada vez que se mida se inserta en este arreglo y al final en el procesamiento, se comprueba la moda.
int moda;
...
void loop(){
  ...
  int coche = getFuzzyValue(medidaLDR(N_MEDIDAS_USO), medidaUltrasonico(N_MEDIDAS_USO));
...
int medidaLDR(int repeticiones){
  ...
  for(int i=0;i<repeticiones;i++){
   ...
   //   Serial.println("termina look up");
    ...
    Serial.print(medidas[repeticiones/2]);
    Serial.println(" lux");
    Serial.print("MEDIA: distancia ");
   // Serial.print(media);
   int resultado=media/repeticiones;
...
int medidaUltrasonico(int repeticiones){
   //inicializacion de media,mediana y moda al comenzar el ciclo de medicion.
  media=0;
  //moda=0;
  for(int i=0;i<N_MODAS;i++){
    valoresModa[i]=0;
  }
  for(int i=0;i<repeticiones;i++){
    medidas[i]=0;
  ...
  for(int i=0;i<repeticiones;i++){
      ...
      int mediaMovilUltra[ESCENARIOS_CALIBRAR];
    ...
    Serial.print("MEDIANA: distancia ");
    Serial.print(medidas[repeticiones/2]);
    resultado = medidas[repeticiones/2]; //modificado
    Serial.println(" cm");
    Serial.print("MEDIA: distancia ");
   // Serial.print(media);
    //resultado=media/repeticiones; //modificado
    Serial.print(resultado);
    Serial.println(" cm");
...
void calibrar(){
  for(int i=0;i<ESCENARIOS_CALIBRAR;i++){
    ...
    /* Wire.write("ok");
     ...
     Serial.println("calibrando LDR");
     mediasFoto[i]=medidaLDR(N_MEDIDAS_USO);
     saveEEPROM(mediasFoto[i], dirLDR+ i*4); //modificado
     Serial.println("calibrando ULTRA");
     mediasUltra[i]=medidaUltrasonico(N_MEDIDAS_USO);  
